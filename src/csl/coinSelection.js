...
// import { Loader } from 'src/csl/loader'
// https://github.com/berry-pool/nami/blob/main/src/api/loader.js
...
/**
 * @typedef {number} int - Coin Selection algorithm return
 */
 ...
 * @property {string} coinsPerUtxoWord
 * @property {string} minFeeA
 * @property {string} minFeeB
 * @property {string} maxTxSize
 */

/**
 * @type {ProtocolParameters | null}
 */
 ...
 * @param {T | null} obj
 */
function assertNonNull(obj, msg = `Object ${obj} is null!`) {
  if (obj != null) {
    return
  }
  throw new Error(msg)
...
const CoinSelection = {
   ...
   * @return {Promise<SelectionResult>} - Coin Selection algorithm return
   */
  randomImprove: async (inputs, outputs, limit) => {
    assertNonNull(protocolParameters, 'Protocol parameters not set. Use setProtocolParameters().')
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      let range = {};
      range.ideal = LCSL.Value.new(
        LCSL.BigNum.from_str('0')
      )
        .checked_add(splitOutputsAmounts[i])
        .checked_add(splitOutputsAmounts[i]);
      range.maximum = LCSL.Value.new(
        LCSL.BigNum.from_str('0')
      )
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let minAmount = LCSL.Value.new(
        LCSL.min_ada_required(
          change,
          false,
          LCSL.BigNum.from_str(protocolParameters.coinsPerUtxoWord)
        )
      );
      ...
      if (compare(change, minAmount) < 0) {
        ...
        const minAda = minAmount
          .checked_sub(LCSL.Value.new(change.coin()))
          .checked_add(LCSL.Value.new(utxoSelection.amount.coin()));
...
function improve(utxoSelection, outputAmount, limit, range) {
  ...
  const newAmount = LCSL.Value.new(
    LCSL.BigNum.from_str('0')
  )
...
function mergeOutputsAmounts(outputs) {
  let compiledAmountList = LCSL.Value.new(
    LCSL.BigNum.from_str('0')
  );
...
function splitAmounts(amounts) {
  ...
  if (amounts.multiasset() && amounts.multiasset().len() > 0) {
    ...
    for (let i = 0; i < mA.keys().len(); i++) {
      ...
      for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
        let _assets = LCSL.Assets.new();
        let assetName = mA.get(scriptHash).keys().get(j);

        _assets.insert(
          LCSL.AssetName.from_bytes(assetName.to_bytes()),
          LCSL.BigNum.from_bytes(
            mA.get(scriptHash).get(assetName).to_bytes()
          )
        );

        let _multiasset = LCSL.MultiAsset.new();
        _multiasset.insert(
          LCSL.ScriptHash.from_bytes(scriptHash.to_bytes()),
          _assets
        );
        let _value = LCSL.Value.new(
          LCSL.BigNum.from_str('0')
        );
  ...
  splitAmounts.push(
    LCSL.Value.new(
      LCSL.BigNum.from_bytes(amounts.coin().to_bytes())
    )
  );
...
function isQtyFulfilled(outputAmount, cumulatedAmount, nbFreeUTxO) {
  assertNonNull(protocolParameters)
  ...
  if (!outputAmount.multiasset() || outputAmount.multiasset().len() <= 0) {
    ...
    if (nbFreeUTxO > 0) {
      ...
      const maxFeeVal = LCSL.Value.new(
        LCSL.BigNum.from_str(maxFee.toString())
      );

      amount = amount.checked_add(maxFeeVal);
    }
  }
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) =>
    LCSL.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  );
 ...
 */
const cloneValue = (value) => LCSL.Value.from_bytes(value.to_bytes());
...
function createEmptyValue() {
  const value = LCSL.Value.new(LCSL.BigNum.from_str('0'));
